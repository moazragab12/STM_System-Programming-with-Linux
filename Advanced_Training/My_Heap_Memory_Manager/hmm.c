            ...
            if (current->free == yes && current->size >= size) {
                if (current->size < size + node_size) {
                    ...
                    if (NULL == best_fit) //if it is the first block we find that is larger than the requested size
                        ...
                        //if we find a block that is larger than the requested size and smaller than the previous best fit
                        best_fit = current;
                }
            }
...
void merge(node *nodes) {
    ...
    if (nodes->next == NULL && nodes->free == yes
        ...
        if (temp == nodes->size) {
            ...
            long long totalsize = temp + node_size;
            void *check;
            while (totalsize > size_t_Max) {
                // DeAllocate the maximum size_t value if the requested size
                // is too large
                check = sbrk(-size_t_Max);
                totalsize -= size_t_Max;
                if (check == (void *) -1) {
                    perror(" sbrk failed ");
                }
            }
...
//                (void *) current->next, current->size, current->free);
