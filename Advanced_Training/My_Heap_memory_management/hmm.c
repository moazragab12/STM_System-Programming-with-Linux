...
// there is an error accessing out of boundry here

	// if((__intptr_t)current >(__intptr_t)upper_sbrk || (__intptr_t)current<(__intptr_t)heap) {

	//      previos->next=(char *)((__intptr_t)previos+sizeof(node)+size);

	//      current=(node *)previos->next;

	// }

//trying to access after tail error
...
// error next is larger than upper_sbrk
	...
	// error  next is referening to previous
...

{
    ...
    void *pointer = heap_allocator(size);

    if (pointer != NULL) {

	memcpy(pointer, ptr, ((node *) (ptr - node_size))->size);
